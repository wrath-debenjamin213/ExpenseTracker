...
class ModernExpenseTracker:
    ...
    def create_entry(self, parent, row, column):
        entry = tk.Entry(parent, font=self.fonts["body"], bd=0,
                         bg="#3a3a4a" if self.current_theme == "dark" else "#e0e0e0",
                         fg=self.colors["text"], insertbackground=self.colors["text"],
                         relief="flat", highlightthickness=1,
                         highlightbackground=self.colors["background"], highlightcolor=self.colors["accent"])
        entry.grid(row=row, column=column, sticky="ew", pady=10, ipady=6)
        return entry

    def create_login_frame(self):
        self.login_frame = tk.Frame(self.root, bg=self.colors["background"])
        header = tk.Frame(self.login_frame, bg=self.colors["primary"], height=80)
        header.pack(fill="x")
        tk.Label(header, text="🔒 WELCOME TO TRACKWISE", font=self.fonts["title"],
                 bg=self.colors["primary"], fg="white", pady=10).pack()

        login_container = tk.Frame(self.login_frame, bg=self.colors["card"], padx=20, pady=20)
        login_container.pack(pady=50, padx=30, fill="both")

        tk.Label(login_container, text="USERNAME", font=self.fonts["body"],
                 bg=self.colors["card"], fg=self.colors["text"]).grid(row=0, column=0, sticky="w", pady=10)
        self.username_entry = self.create_entry(login_container, 0, 1)

        tk.Label(login_container, text="PASSWORD", font=self.fonts["body"],
                 bg=self.colors["card"], fg=self.colors["text"]).grid(row=1, column=0, sticky="w", pady=10)
        self.password_entry = self.create_entry(login_container, 1, 1)
    ...
    def create_nav_bar(self):
        ...
        self.view_menu.add_command(label="💹 SIP Tracker", command=lambda: self.show_frame(self.sip_tracker_frame))
    ...
    def create_main_frame(self):
        ...
        cards_frame = tk.Frame(self.main_frame, bg=self.colors["background"], padx=20, pady=20)
        cards_frame.pack(fill="both", expand=True)

        cards_frame.grid_columnconfigure((0, 1, 2), weight=1)
        cards_frame.grid_rowconfigure((0, 1), weight=1)

        total_card = self.create_card(cards_frame, "TOTAL EXPENSES", "₹0.00",
                                     self.colors["success"], 0, 0)
        self.total_label = total_card[1]

        recent_card = self.create_card(cards_frame, "RECENT EXPENSE", "No recent expenses",
                                      self.colors["accent"], 0, 1)
        self.recent_label = recent_card[1]

        budget_card = self.create_card(cards_frame, "MONTHLY BUDGET", "₹0.00 / ₹0.00",
                                      self.colors["info"], 0, 2)
        self.budget_label = budget_card[1]

        category_card = self.create_card(cards_frame, "TOP CATEGORY", "No data",
                                        self.colors["highlight"], 1, 0)
        self.cat_label = category_card[1]

        sip_card = self.create_card(cards_frame, "TOTAL SIP", "₹0.00",
                                   self.colors["info"], 1, 1)
        self.sip_total_label = sip_card[1]

        daily_card = self.create_card(cards_frame, "DAILY BUDGET", "₹0.00 / ₹0.00",
                                     self.colors["warning"], 1, 2)
        self.daily_label = daily_card[1]
    ...
    def create_card(self, parent, title, value, color, row, column):
        card = tk.Frame(parent, bg=self.colors["card"], padx=15, pady=10)
        card.grid(row=row, column=column, padx=10, pady=10, sticky="nsew")
        tk.Label(card, text=title, font=self.fonts["subheader"],
    ...
    def create_add_expense_frame(self):
        ...
        tk.Label(form_frame, text="AMOUNT (₹)", font=self.fonts["body"],
                 bg=self.colors["card"], fg=self.colors["text"]).grid(row=0, column=0, sticky="w", pady=10)
        self.amount_entry = self.create_entry(form_frame, 0, 1)
        ...
        self.category_dropdown.grid(row=1, column=1, sticky="ew", pady=10, ipady=5)
        self.category_dropdown.current(0)

        tk.Label(form_frame, text="DATE (YYYY-MM-DD)", font=self.fonts["body"],
                 bg=self.colors["card"], fg=self.colors["text"]).grid(row=2, column=0, sticky="w", pady=10)
        self.date_entry = self.create_entry(form_frame, 2, 1)
        self.date_entry.insert(0, datetime.now().strftime("%Y-%m-%d"))

        tk.Label(form_frame, text="DESCRIPTION", font=self.fonts["body"],
                 bg=self.colors["card"], fg=self.colors["text"]).grid(row=3, column=0, sticky="w", pady=10)
        self.desc_entry = self.create_entry(form_frame, 3, 1)
    ...
    def create_settings_frame(self):
        ...
        theme_dropdown.grid(row=0, column=1, sticky="ew", pady=10, ipady=5)
        ...
        account_frame.grid(row=1, column=1, sticky="ew", pady=10)
    ...
    def create_sip_tracker_frame(self):
        ...
        header.pack(fill="x")
        tk.Label(header, text="💹 SIP TRACKER", font=self.fonts["title"],
                 bg=self.colors["primary"], fg="white", pady=10).pack()

        add_frame = tk.Frame(self.sip_tracker_frame, bg=self.colors["card"], padx=20, pady=20)
        add_frame.pack(padx=30, pady=(20, 10), fill="x")

        tk.Label(add_frame, text="AMOUNT (₹)", font=self.fonts["body"],
                 bg=self.colors["card"], fg=self.colors["text"]).grid(row=0, column=0, sticky="w", pady=10)
        self.sip_amount_entry = self.create_entry(add_frame, 0, 1)
        ...
        self.sip_category_dropdown.grid(row=1, column=1, sticky="ew", pady=10, ipady=5)
        ...
        self.sip_freq_dropdown.grid(row=2, column=1, sticky="ew", pady=10, ipady=5)
        self.sip_freq_dropdown.current(0)

        tk.Label(add_frame, text="START DATE (YYYY-MM-DD)", font=self.fonts["body"],
                 bg=self.colors["card"], fg=self.colors["text"]).grid(row=3, column=0, sticky="w", pady=10)
        self.sip_date_entry = self.create_entry(add_frame, 3, 1)
        self.sip_date_entry.insert(0, datetime.now().strftime("%Y-%m-%d"))

        tk.Label(add_frame, text="NAME", font=self.fonts["body"],
                 bg=self.colors["card"], fg=self.colors["text"]).grid(row=4, column=0, sticky="w", pady=10)
        self.sip_name_entry = self.create_entry(add_frame, 4, 1)
        ...
        table_frame = tk.Frame(self.sip_tracker_frame, bg=self.colors["background"], padx=20, pady=15)
        table_frame.pack(fill="both", expand=True)
        ...
        self.sip_tree.pack(side="left", fill="both", expand=True)
        scrollbar.pack(side="right", fill="y")
        ...
        back_frame = tk.Frame(self.sip_tracker_frame, bg=self.colors["background"], padx=20, pady=15)
        back_frame.pack(fill="x")
        self.create_button(back_frame, "🔙 BACK",
                          lambda: self.show_frame(self.main_frame),
                          self.colors["secondary"]).pack(side="right", ipadx=8, ipady=4)
    ...
    def create_budget_frame(self):
        ...
        form_frame = tk.Frame(self.budget_frame, bg=self.colors["card"], padx=20, pady=20)
        form_frame.pack(padx=30, pady=20, fill="both", expand=True)

        tk.Label(form_frame, text="MONTHLY BUDGET (₹)", font=self.fonts["body"],
                 bg=self.colors["card"], fg=self.colors["text"]).grid(row=0, column=0, sticky="w", pady=10)
        self.monthly_budget_entry = self.create_entry(form_frame, 0, 1)
        self.monthly_budget_entry.insert(0, str(self.user_data[self.current_user]['monthly_budget']))

        tk.Label(form_frame, text="DAILY BUDGET (₹)", font=self.fonts["body"],
                 bg=self.colors["card"], fg=self.colors["text"]).grid(row=1, column=0, sticky="w", pady=10)
        self.daily_budget_entry = self.create_entry(form_frame, 1, 1)
        self.daily_budget_entry.insert(0, str(self.user_data[self.current_user]['daily_budget']))

        tk.Label(form_frame, text="MONTHLY GOALS BY CATEGORY", font=self.fonts["header"],
                 bg=self.colors["card"], fg=self.colors["text"]).grid(row=2, column=0, columnspan=2, pady=10)

        self.goal_entries = {}
        for i, category in enumerate(self.categories, start=3):
            tk.Label(form_frame, text=f"{category}:", font=self.fonts["body"],
                     bg=self.colors["card"], fg=self.colors["text"]).grid(row=i, column=0, sticky="w", pady=5)
            entry = tk.Entry(form_frame, font=self.fonts["body"], bd=0,
                            bg="#3a3a4a" if self.current_theme == "dark" else "#e0e0e0",
                            fg=self.colors["text"], insertbackground=self.colors["text"],
                            relief="flat", highlightthickness=1,
                            highlightbackground=self.colors["background"], highlightcolor=self.colors["accent"])
        ...
        button_frame = tk.Frame(form_frame, bg=self.colors["card"])
        button_frame.grid(row=len(self.categories) + 3, column=0, columnspan=2, pady=20)
    ...
    def save_budget(self):
        try:
            ...
            self.save_user_data()
            self.show_custom_message("SUCCESS", "Budget settings saved! 🎉", "success")
    ...
    def save_expense(self):
        try:
            ...
            self.show_custom_message("SUCCESS", "Expense saved! 💾", "success")
            self.update_dashboard()
    ...
    def save_sip(self):
        try:
            ...
            self.update_charts()
            self.clear_sip_form()
            self.show_custom_message("SUCCESS", "SIP added! 💹", "success")
        except ValueError as e:
            self.show_custom_message("ERROR", f"🚫 {str(e)}", "error")
    ...
    def update_dashboard(self):
        ...
        if expenses:
            ...
            self.recent_label.config(text="No recent expenses")
        ...
        if expenses:
            ...
            if cat_counts:
                ...
                self.cat_label.config(text=top_cat)
        else:
            self.cat_label.config(text="No data")

        sip_total = sum(sip['amount'] for sip in sips)
        self.sip_total_label.config(text=f"₹{sip_total:,.2f}")
        ...
        self.budget_label.config(text=f"₹{monthly_spent:,.2f} / ₹{user['monthly_budget']:,.2f}")
        self.daily_label.config(text=f"₹{daily_spent:,.2f} / ₹{user['daily_budget']:,.2f}")
    ...
    def edit_expense(self):
        ...
        amount_entry = self.create_entry(form_frame, 0, 1)
        amount_entry.insert(0, expense['amount'])

        tk.Label(form_frame, text="CATEGORY", font=self.fonts["body"],
                 bg=self.colors["card"], fg=self.colors["text"]).grid(row=1, column=0, sticky="w", pady=5)
        category_var = tk.StringVar(value=expense['category'])
        category_dropdown = ttk.Combobox(form_frame, textvariable=category_var,
                                        values=self.categories, font=self.fonts["body"])
        category_dropdown.grid(row=1, column=1, sticky="ew", pady=5, ipady=5)

        tk.Label(form_frame, text="DATE (YYYY-MM-DD)", font=self.fonts["body"],
                 bg=self.colors["card"], fg=self.colors["text"]).grid(row=2, column=0, sticky="w", pady=5)
        date_entry = self.create_entry(form_frame, 2, 1)
        date_entry.insert(0, expense['date'])

        tk.Label(form_frame, text="DESCRIPTION", font=self.fonts["body"],
                 bg=self.colors["card"], fg=self.colors["text"]).grid(row=3, column=0, sticky="w", pady=5)
        desc_entry = self.create_entry(form_frame, 3, 1)
        ...
        def save_changes():
            try:
                ...
                self.update_charts()
                edit_window.destroy()
                self.show_custom_message("SUCCESS", "Expense updated! ✏️", "success")
                self.check_budget_alerts()
            except ValueError as e:
    ...
    def delete_expense(self):
        ...
        self.update_dashboard()
        self.update_charts()
        self.show_custom_message("SUCCESS", "Expense deleted! 🗑️", "success")
        self.check_budget_alerts()
    ...
    def delete_sip(self):
        ...
        self.update_dashboard()
        self.update_charts()
        self.show_custom_message("SUCCESS", "SIP deleted! 🗑️", "success")
    ...
    def export_data(self):
        ...
        if file_path:
            try:
                ...
                self.show_custom_message("SUCCESS", "Data exported! 📤", "success")
    ...
    def import_data(self):
        ...
        if file_path:
            try:
                ...
                self.update_dashboard()
                self.update_charts()
                self.show_custom_message("SUCCESS", "Data imported! 📥", "success")
                self.check_budget_alerts()
            except Exception as e:
    ...
    def change_theme(self, event=None):
        ...
        current_frame = None
        for frame, create_func in frames:
    ...
    def show_frame(self, frame):
        ...
        if frame == self.view_expenses_frame:
            ...
            self.update_charts()
        elif frame == self.sip_tracker_frame:
            self.update_sip_table()
